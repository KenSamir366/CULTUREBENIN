...
<head>
    ...
    <title>Modifier Parler - Culture_Benin</title>
                ...
                <div class="card">
                    <div class="card-header">
                        <h4 class="mb-0">
                            ...
                            Modifier l'association Région-Langue
                        </h4>
                    </div>
                    <div class="card-body">
                        <form action="{{ route('parlers.update', $parler->id) }}" method="POST">
                                ...
                                <select class="form-select @error('region_id') is-invalid @enderror" 
                                    ...
                                    @foreach($regions as $region)
                                        <option value="{{ $region->id }}" 
                                            {{ $parler->region_id == $region->id ? 'selected' : '' }}>
                                            {{ $region->nom_regions }}
                                        </option>
                                    @endforeach
                                </select>
                                ...
                                <select class="form-select @error('langue_id') is-invalid @enderror" 
                                    ...
                                    @foreach($langues as $langue)
                                        <option value="{{ $langue->id }}" 
                                            {{ $parler->langue_id == $langue->id ? 'selected' : '' }}>
                                            {{ $langue->nom }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="mt-4">
                                ...
                                <a href="{{ route('parlers.show', $parler->id) }}" class="btn btn-secondary">
